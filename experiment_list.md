...
- **Two compression metrics reported**: coefficients-only (`comp_ratio = P³/k`) and amortised-basis (`comp_ratio = P³ / (k + k×P³/n_patches)`) — the gap between them is large for small tile counts and negligible for large datasets.
